    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="bg-[#F9F1E7] py-8">
        <div className="max-w-7xl mx-auto px-4 flex items-center gap-4 text-sm">
          <Link to="/" className="text-gray-500 hover:text-[#B88E2F]">Home</Link>
          <span className="font-bold text-gray-800">&gt;</span>
          <Link to="/shop" className="text-gray-500 hover:text-[#B88E2F]">Shop</Link>
          <span className="font-bold text-gray-800">&gt;</span>
          ...
          <div className="flex flex-col gap-6 w-20">
            {[1,2,3,4].map((i) => (
              <div key={i} className="w-20 h-20 bg-[#F9F1E7] rounded-lg overflow-hidden flex items-center justify-center cursor-pointer border border-transparent hover:border-[#B88E2F]">
                <img src={product.image} alt="Thumbnail" className="w-full h-full object-cover mix-blend-multiply opacity-80" />
              </div>
            ))}
          </div>
          {/* Main Image */}
          <div className="flex-1 bg-[#F9F1E7] rounded-lg overflow-hidden flex items-center justify-center relative">
             <img src={product.image} alt={product.name} className="w-[80%] h-auto object-contain mix-blend-multiply" />
          </div>
        </div>

        {/* Product Details Right Column */}
        <div className="flex flex-col">
          <h1 className="text-4xl font-normal mb-2 text-gray-900">{product.name}</h1>
          <p className="text-2xl text-gray-500 font-medium mb-4">₹{product.price.toLocaleString()}.00</p>
          ...
          <div className="flex items-center gap-4 mb-12">
            <div className="flex items-center border border-gray-400 rounded-lg px-4 py-3 w-32 justify-between">
              <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="text-gray-500 hover:text-black text-lg font-bold">-</button>
              <span className="font-medium text-gray-900">{quantity}</span>
              <button onClick={() => setQuantity(quantity + 1)} className="text-gray-500 hover:text-black text-lg font-bold">+</button>
            </div>
            
            <button
              onClick={handleAddToCart}
              className="border border-black rounded-lg px-10 py-3 text-gray-900 font-medium hover:bg-black hover:text-white transition-colors"
            >
              Add To Cart
            </button>
            ...
            <button
              ...
              className="border border-[#B88E2F] rounded-lg px-10 py-3 text-[#B88E2F] font-medium hover:bg-[#B88E2F] hover:text-white transition-colors"
            >
              Buy Now
            </button>
          </div>
          ...
          <div className="border-t border-gray-200 pt-10 text-gray-400 text-sm flex flex-col gap-3">
            <div className="grid grid-cols-[100px_10px_auto]">
              <span>SKU</span><span>:</span><span>SS00{product.id}</span>
            </div>
            <div className="grid grid-cols-[100px_10px_auto]">
        ...
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-10 text-left">
            {relatedProducts.map((prod) => (
              <Link to={`/comparison?id=${prod.id}`} key={prod.id} className="bg-[#F4F5F7] group relative block">
                {/* Product Image */}
                <div className="relative h-[300px] w-full overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-4">
                    <button
                      onClick={(e) => { e.preventDefault(); addToCart({ id: prod.id, name: prod.name, price: prod.price, image: prod.image }); }}
                      className="bg-white text-[#B88E2F] px-10 py-3 font-semibold w-[80%] hover:bg-[#B88E2F] hover:text-white transition-colors"
                    >
                      Add to cart
                    </button>
                ...
                <div className="p-4 pt-4 pb-6">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xl font-bold text-gray-900">₹{prod.price.toLocaleString()}</span>
                  </div>
                </div>
              </Link>
            ))}
          ...
          <Link
            to="/shop"
            className="inline-block border border-[#B88E2F] text-[#B88E2F] bg-white px-16 py-3 font-semibold hover:bg-[#B88E2F] hover:text-white transition-colors"
          >
            Show More
          </Link>
        </div>
